...
Facts (trunk, check command, board, labels, docs paths, vocabulary) come from **`AGENTS.md` → Repo facts**. A skill that needs a fact not listed there derives it (`gh repo view`, `git`, the manifest), states it in one message, and confirms once. Never invent.

## Contract 2 — the slug

`GH-<N>-<kebab-topic>` once an issue exists, else `<kebab-topic>`. Brainstorm, spec, plan, checklist and branch share it. A skill that finds an earlier-stage file for the slug reads it first. Issue titles carry the topic, so keep them kebab-able (`Undo by reply drops the wrong entry`, not `Bug`).
...
- **Never destroy work.** No `reset --hard`, no force push, no `git add -A` unasked, stop on divergence.
- **No fabricated references.** Every `file:line`, issue number and PR number was read this run.
- **Privacy is a rule, not a preference.** Nothing from the author's personal ledger, vault, VPS, tokens or creator id enters this repo, an issue, a PR or a doc. Sample data is fake and regenerated by `tests/make_sample.py`.

## Contract 4 — one owner per board transition

Project board: `mhmzdev/projects/1` ("Hisab Engineering"), Status options `Todo · In progress · Done`.
...
`create-plan` and `review` do not move the card (the board has no planned / in-review column). A plan is a contract: `create-plan` leaves no open question; `implement` refuses a plan that carries one.
