...
|---|---|---|
| [GH-43-transfer-direction](completed/GH-43-transfer-direction.md) | 2026-09-20 | Setup learns the account holder name (question 2 of 8, every onboarding message numbered `n/8`); the prompt carries one direction ladder for typed text, SMS and photos; `tests/check_receipts.py` checks the model by hand against three fixtures the owner places |
| [GH-65-setup-currency](completed/GH-65-setup-currency.md) | 2026-09-20 | The currency answered in setup survives a restart: `Ledger.currency` is a read-only property that reads `settings.json` (a valid `[A-Z]{3,4}` code) and falls back to config.yaml's `default_currency` before setup or on a missing or malformed value; `settings()` never crashes on a non-dict or non-UTF-8 file; self-host and hosted restarts proven in smoke; the VPS guide no longer asks for the currency in config.yaml |
| [GH-68-agent-token](completed/GH-68-agent-token.md) | 2026-09-20 | The WhatsApp token is read from `WHATSAPP_AGENT_TOKEN` (wa-agent's name); `WHATSAPP_TOKEN` still works, with a once-per-process note to rename it; an empty or whitespace-only new name falls back. The runner strips both names and `RUNNER_PRIVATE_KEY` from each worker, hands it only its tenant's token as `WHATSAPP_AGENT_TOKEN`, and sets `HISAB_NO_DOTENV=1`, so a tenant worker never reads a `.env`; the state hash is unchanged for running tenants. Docs switched |
