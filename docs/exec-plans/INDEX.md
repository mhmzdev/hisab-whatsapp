...
|---|---|---|
| [GH-22-landing-two-languages](completed/GH-22-landing-two-languages.md) | 2026-09-14 | Landing and portal ship exactly `en` and `ur`: switcher drops Roman Urdu, a saved `roman` falls back to English, 104 `roman` values removed from `strings.json`, `check_landing.py` fails on a missing or extra language (smoke asserts both); `hisab/i18n.py` keeps three languages |
