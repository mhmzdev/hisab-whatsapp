...
4. Clone the repo into ~/hisab-whatsapp and work only from there. Copy .env.example to .env and config.example.yaml to config.yaml. Do not ask me to paste a key into this chat. Tell me to open ~/hisab-whatsapp/.env on the server myself and fill in WHATSAPP_AGENT_TOKEN (my WhatsApp agent's API key) and either OPENROUTER_API_KEY or GEMINI_API_KEY (one is enough; the config uses whichever is set), then run chmod 600 .env. Wait until I say it is done. Then check that the lines are filled without printing them, for example: grep -cE '^(WHATSAPP_AGENT_TOKEN|WHATSAPP_TOKEN|OPENROUTER_API_KEY|GEMINI_API_KEY)=.+' .env counts the filled lines (2 or more is right; WHATSAPP_TOKEN is the old name for WHATSAPP_AGENT_TOKEN and still works).
5. ASK my timezone as an IANA name (for example Asia/Karachi). Set timezone: in config.yaml. Leave everything else at its default. The server clock is usually UTC and can stay UTC, because Hisab reads its dates only from timezone in config.yaml. Tell me that setup in WhatsApp asks for my currency code, and the one I give there is the one Hisab keeps.
