...
hisab/
  ...
  config.py      config.yaml + .env (secrets only from the environment); the WhatsApp token is read from
                 WHATSAPP_AGENT_TOKEN, else WHATSAPP_TOKEN (the old name) with a note to rename it
